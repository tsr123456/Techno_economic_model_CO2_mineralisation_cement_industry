...
%% Calculations------------------------------------------------------------
function [numbr_reactor, size_reactor,count_reactor, tw_reactor] = design_reactor(tw_vessel_max,maximum_vessel_size,v_vessel,p_vessel,sf_vessel,mat_vessel,je_vessel,d_vessel,t_vessel,ASME)
...
p_reactor = p_vessel*10^5; %pressure of reaction (importet in bar and adjusted to Pa)
    ...
    [~, closestIndex] = min(abs(ASME(1,:)-t_vessel));
    ms_reactor =  ASME(2+mat_vessel,closestIndex)*10^6;
...
[~, closestIndex] = min(abs(ASME(4,:)-di_reactor)); % find closest index to chosen diameter of reactor
tw_reactor = max ((ASME(5,closestIndex)/1000),tw_reactor); % compare calculated pressure dependent min wall thickness to constructing dependend min wall thickness
