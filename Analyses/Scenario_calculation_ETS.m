...
pc_ETS_sensi = linspace(0,200,numbr_of_model_runs);

distance_transport = 1200;%alter transport distance here.
%% ------------------------------Direct Process------------------------- %%
...
%limit transport
I1_initial(48,1) = distance_transport;
...
%% -------------------Indirect Process--------------------------------- %%
clearvars -except Result_C_total_fu_direct Result_Rev_total_fu_direct Result_Profit_total_direct numbr_of_model_runs Fu_cement_replacement SENSI_NUM SENSI_TXT numbr_scenarios GHG_reduction_direct CO2_captured_direct pc_ETS_sensi SENSI_CAP distance_transport
...
[Result_total_cost_fu_indirect(:,v), Result_total_rev_fu_indirect(:,v), Result_total_profit_indirect(:,v), GHG_reduction_indirect(:,v), CO2_captured_indirect(:,v)] = TEA_indirect_standalone(M1,M2,M3,M4,M5,M6,Mass_i_o_TXT, numbr_of_model_runs,Fu_cement_replacement, pc_ETS_sensi, [],[],distance_transport);
...
for i_plot=1:numbr_scenarios  % a loop, plot y1 against each column of X %plotstyle{i_plot+1}, 'LineWidth',3, 'MarkerSize',3
    ...
    for i_area = 1: length(pc_ETS_sensi)
    ...
    % plot postive plot
    q=area(pc_ETS_sensi,positive(:,1),'FaceColor',color_area_pos);
    q.FaceAlpha = area_trans;
    q.EdgeColor = 'none';
    hold on
    q_2 = area(pc_ETS_sensi,positive(:,2),'FaceColor', [1 1 1]);
    q_2.EdgeColor = 'none';
    hold on
    
    
    %plot negative plot
    q=area(pc_ETS_sensi,negative(:,1),'FaceColor',color_area_neg);
    q.FaceAlpha = area_trans;
    q.EdgeColor = 'none';
    hold on
    q_2 = area(pc_ETS_sensi,negative(:,2),'FaceColor', [1 1 1]);
    q_2.EdgeColor = 'none';
    hold on
    
       
    %Plot lines
    p_1(i_plot,1) = plot(tile(i_plot),pc_ETS_sensi,Result_C_total_fu_direct(:,i_plot),'-', 'color',color1, 'LineWidth',2);
      hold on
    p_1(i_plot,2) = plot(tile(i_plot),pc_ETS_sensi,Result_Rev_total_fu_direct(:,i_plot),'--', 'color',color2, 'LineWidth',2);
 ...
 %%%----------------------------Indirect Process------------------------%%%
   ...
   % plot postive plot
    ...
    hold on
            ...
            plot(tile(i_plot+numbr_scenarios),pc_ETS_sensi,Result_total_cost_fu_indirect(:,i_plot),'-','color',color1, 'LineWidth',2);
    hold on
            plot( tile(i_plot+numbr_scenarios),pc_ETS_sensi,Result_total_rev_fu_indirect(:,i_plot), '--', 'color',color2, 'LineWidth',2);
...
xlabel(graph_tiles,'\pi_{ETS} [€/t_{CO2}]', 'FontSize',size_font)
ylabel(graph_tiles,'Cost & revenue [€/t_{SCM}]', 'FontSize',size_font)
